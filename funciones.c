...
void listarPeliculas(char peliculas[10][4][40]) {
    ...
    char decision[3];
    printf("Desea guardar la lista de peliculas en un archivo? (si/no): ");
    scanf("%2s", decision);

    if (strcmp(decision, "si") == 0) {
        FILE *archivoPeliculas;
        archivoPeliculas = fopen("peliculas.txt", "w");
        for (int i = 0; i < 10; i++) {
            fprintf(archivoPeliculas, "%s\t%s\t%s\t%s\n", peliculas[i][0], peliculas[i][1], peliculas[i][2], peliculas[i][3]);
        }

        fclose(archivoPeliculas);
        printf("La lista de peliculas se ha guardado en el archivo 'peliculas.txt'.\n");
    }
}
